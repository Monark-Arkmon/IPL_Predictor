...
class FeatureSelector:
    ...
    def __init__(self, selection_methods: Optional[List[str]] = None):
        self.selection_methods = selection_methods or ['univariate', 'rfe', 'importance']
        self.selectors = {}
        self.selected_features = {}
    
    def select_features(self, X: np.ndarray, y: np.ndarray, 
                       feature_names: List[str], k: int = 20) -> Tuple[np.ndarray, List[str]]:
        """Apply multiple feature selection methods and combine results."""
        logger.info(f"Selecting top {k} features using methods: {self.selection_methods}")
        ...
        if 'univariate' in self.selection_methods:
            ...
            for idx in selected_indices:
                feature_scores[feature_names[idx]] = feature_scores.get(feature_names[idx], 0) + 1
        ...
        selected_features = [name for name, score in feature_scores.items() if score >= min_votes]
...
class CrossValidator:
    ...
    def cross_validate_model(self, model: Any, X: np.ndarray, y: np.ndarray) -> Dict[str, float]:
...
class ModelRegistry:
    ...
    def get_model(self, name: str, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        if name not in self.models:
            raise ValueError(f"Model {name} not supported. Available: {list(self.models.keys())}")
...
class HyperparameterOptimizer:
    ...
    def optimize(self, X: np.ndarray, y: np.ndarray, cv_folds: int = 5) -> Dict[str, Any]:
...
class ModelTrainer:
    ...
    def train_single_model(self, model_name: str, X_train: np.ndarray, X_test: np.ndarray,
                          y_train: np.ndarray, y_test: np.ndarray, 
                          feature_names: Optional[List[str]] = None,
                          optimize_hyperparams: bool = True,
                          use_feature_selection: bool = True) -> Tuple[Any, Dict[str, float]]:
        ...
        with mlflow.start_run(run_name=f"{model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            ...
            if use_feature_selection and feature_names and len(feature_names) > 10:
                ...
                mlflow.log_param("selected_features", selected_features[:10])  # Log first 10
            else:
            ...
            cv_results = cv_validator.cross_validate_model(model, X_train_selected, y_train)
            ...
            try:
                ...
                shap_values = shap_explainer.calculate_shap_values(X_test_selected[:50])  # Sample for speed
                
                if shap_values is not None:
                    shap_importance = shap_explainer.get_feature_importance(selected_features)
                    if shap_importance:
                        # Log top 10 SHAP features
                        sorted_shap = sorted(shap_importance.items(), key=lambda x: x[1], reverse=True)[:10]
                        mlflow.log_params({f"shap_feature_{i}": f"{name}:{score:.4f}" 
            ...
            if hasattr(model, 'feature_names_in_') or not hasattr(model, 'feature_names_in_'):
                model.feature_names_in_ = np.array(selected_features) if selected_features else np.array([])
            ...
            logger.info(f"{model_name} training completed. Accuracy: {metrics['accuracy']:.4f}")
            
            return model, metrics
    
    def train_ensemble_models(self, X_train: np.ndarray, X_test: np.ndarray,
                            y_train: np.ndarray, y_test: np.ndarray,
                            feature_names: Optional[List[str]] = None) -> Dict[str, Tuple[Any, Dict[str, float]]]:
        ...
        for model_name in self.config.models.algorithms[:4]:  # Limit for ensemble
            try:
                model, metrics = self.train_single_model(
                    ...
                    feature_names=feature_names, optimize_hyperparams=True, use_feature_selection=True
                )
        ...
        if len(base_models) >= 2:
            ...
            # Voting ensemble
            with mlflow.start_run(run_name=f"voting_ensemble_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                ...
                metrics = self._calculate_metrics(y_test, np.array(y_pred), y_pred_proba)
                mlflow.log_metrics(metrics)
            ...
            # Stacking ensemble
            with mlflow.start_run(run_name=f"stacking_ensemble_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                ...
                mlflow.log_param("base_models", [name for name, _ in base_models])
                mlflow.log_param("meta_model", type(ensemble_creator.meta_model).__name__)
    ...
    def save_best_model(self, results: Dict[str, Tuple[Any, Dict[str, float]]], 
        ...
        metadata = {
            ...
            'config': {
                'features': len(best_model.feature_names_in_) if hasattr(best_model, 'feature_names_in_') else 'unknown'
            }
        }
        ...
        logger.info(f"Best model ({best_model_name}) saved with {metric}: {best_metrics[metric]:.4f}")
