...
class DataVerificationPipeline:
    ...
    def _verify_data_completeness(self) -> bool:
        ...
        try:
            ...
            unique_teams = len(pd.concat([matches_df['team1'], matches_df['team2']]).unique())
            ...
            date_range = (matches_df['match_date'].max() - matches_df['match_date'].min()).days
            min_days = min_seasons * 365
            
            if date_range < min_days:
                logger.error(f"Insufficient date range: {date_range} days < {min_days} days")
                return False
                
            logger.info(f"Data completeness verified: {len(matches_df)} matches, {unique_teams} teams, {date_range} days")
            return True
    ...
    def _verify_data_consistency(self) -> bool:
        ...
        try:
            ...
            valid_winners = matches_df['winner'].isin(
                pd.concat([matches_df['team1'], matches_df['team2']])
            ).sum()
            
            total_with_winners = len(matches_df.dropna(subset=['winner']))
            consistency_rate = valid_winners / total_with_winners if total_with_winners > 0 else 0
    ...
    def get_verification_summary(self) -> str:
        ...
        overall = "READY FOR TRAINING" if all(self.results.values()) else "FIX ISSUES BEFORE TRAINING"
