...
class DataValidator:
    ...
    def validate_dataframe(self, df: pd.DataFrame, name: str) -> bool:
        ...
        if missing_ratio > max_missing:
            logger.warning(
                f"{name} has {missing_ratio:.2%} missing values (threshold: {max_missing:.2%})"
            )

        # Check data types
        object_cols = df.select_dtypes(include=["object"]).columns
        logger.info(
            f"{name} - Shape: {df.shape}, Missing: {missing_ratio:.2%}, "
    ...
    def validate_team_data(self, df: pd.DataFrame) -> bool:
        ...
        if low_data_teams:
            logger.warning(
                f"Teams with insufficient data (<{min_matches} matches): {low_data_teams}"
            )
...
class FeatureEngineer:
    ...
    def _calculate_team_statistics(self, matches_df: pd.DataFrame) -> pd.DataFrame:
        ...
        total_matches = (
            team_matches.groupby("team").size().reset_index(name="total_matches")
        )

        team_stats = total_matches.merge(
            team_wins, left_on="team", right_on="winner", how="left"
        )
    ...
    def _calculate_batting_features(self, deliveries_df: pd.DataFrame) -> pd.DataFrame:
        ...
        batting_stats = (
            deliveries_df.groupby("batting_team")
            .agg(
                {
                    "total_runs": ["sum", "mean"],
        ...
        batting_stats["avg_strike_rate"] = (
            batting_stats["batsman_runs_sum"] / batting_stats["balls_faced"]
        ) * 100
        ...
        powerplay_runs = (
            deliveries_df[deliveries_df["over"] <= 6]
        ...
        death_runs = (
            deliveries_df[deliveries_df["over"] >= 16]
            .groupby("batting_team")["total_runs"]
            .mean()
        )
        ...
        player_diversity = (
            deliveries_df[deliveries_df["batsman_runs"] > 0]
    ...
    def _calculate_bowling_features(self, deliveries_df: pd.DataFrame) -> pd.DataFrame:
        ...
        bowling_stats = (
            deliveries_df.groupby("bowling_team")
            .agg(
                {"total_runs": ["sum", "mean"], "is_wicket": "sum", "match_id": "count"}
            )
            .round(3)
        )
        ...
        bowling_stats["economy_rate"] = (
            bowling_stats["runs_conceded_sum"] / bowling_stats["balls_bowled"]
        ) * 6
        bowling_stats["wicket_rate"] = (
            bowling_stats["wickets_taken"] / bowling_stats["balls_bowled"]
        )
        ...
        bowling_stats["bowling_strike_rate"] = bowling_stats[
            "bowling_strike_rate"
        ...
        powerplay_econ = (
            deliveries_df[deliveries_df["over"] <= 6]
            .groupby("bowling_team")["total_runs"]
            .mean()
            * 6
        )
    ...
    def _calculate_recent_form(
        self, matches_df: pd.DataFrame, window: int = 5
    ) -> pd.DataFrame:
    ...
    def _calculate_venue_features(self, matches_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for team in pd.concat([matches_df["team1"], matches_df["team2"]]).unique():
            ...
            venue_consistency = (
                np.std(venue_win_rates) if len(venue_win_rates) > 1 else 0
            )

            # Home venue identification (most frequently played venue)
            home_venue = (
                max(venue_performance.items(), key=lambda x: x[1]["total"])[0]
                if venue_performance
                else None
            )
            home_venue_rate = (
                venue_performance[home_venue]["wins"]
                / venue_performance[home_venue]["total"]
                if home_venue
                else 0
            )
            ...
            venue_stats.append(
                {
                    ...
                    "avg_city_performance": (
                        np.mean(city_win_rates) if city_win_rates else 0
                    ),
                }
            )
    ...
    def _calculate_player_impact_features(
        ...
        for team in deliveries_df["batting_team"].unique():
            ...
            batsman_runs = team_deliveries.groupby("batter")["batsman_runs"].agg(
                ["sum", "mean", "count"]
            )
            if not batsman_runs.empty:
                top_batsman = batsman_runs.loc[batsman_runs["sum"].idxmax()]
                avg_top_batsman_contribution = (
                    top_batsman["sum"] / batsman_runs["sum"].sum()
                )
        ...
        for team in deliveries_df["bowling_team"].unique():
            ...
            bowler_wickets = team_bowling.groupby("bowler")["is_wicket"].agg(
                ["sum", "count"]
            )
            bowler_economy = team_bowling.groupby("bowler").apply(
                lambda x: (
                    (x["total_runs"].sum() / x["match_id"].count()) * 6
                    if x["match_id"].count() > 0
                    else 999
                )
            )
    ...
    def _calculate_head_to_head_features(
        ...
        for team in teams:
                ...
                for opponent in frequent_opponents:
                    opponent_matches = matches_df[
                        (
                            ...
                            & (matches_df["team2"] == opponent)
                        )
                        | (
                            (matches_df["team1"] == opponent)
                            & (matches_df["team2"] == team)
                        )
                    ]
                    h2h_total += len(opponent_matches)
                    h2h_wins += len(
                        opponent_matches[opponent_matches["winner"] == team]
                    )
            ...
            team_win_rates = (
                matches_df.groupby("winner").size()
                / matches_df.groupby(["team1", "team2"])
                .size()
                .groupby(["team1", "team2"])
                .first()
                .reset_index()
            ...
            h2h_stats.append(
                {
                    ...
                    "most_played_opponent_rate": (
                        opponent_counts.iloc[0] / len(team_matches)
                        if len(opponent_counts) > 0
                        else 0
                    ),
                }
            )
    ...
    def create_match_features(
        self, matches_df: pd.DataFrame, team_features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        match_features["win_rate_diff"] = (
            match_features["win_rate_team1"] - match_features["win_rate_team2"]
        )
        match_features["form_diff"] = (
            match_features["recent_form_team1"] - match_features["recent_form_team2"]
        )
        match_features["batting_strength_diff"] = (
            match_features["avg_strike_rate_team1"]
            - match_features["avg_strike_rate_team2"]
        )
        match_features["bowling_strength_diff"] = (
            match_features["economy_rate_team2"] - match_features["economy_rate_team1"]
        )  # Lower is better
        ...
        match_features["is_playoff"] = np.where(
            match_features["season"].notna(),
            0,
            0,  # Placeholder - would need playoff identification logic
        )

        # Remove intermediate columns
        cols_to_drop = [
            col for col in match_features.columns if col.endswith(("_team1", "_team2"))
        ]
        team_cols_to_drop = ["team_team1", "team_team2"]
        match_features = match_features.drop(
            columns=[
                col
                for col in cols_to_drop + team_cols_to_drop
    ...
    def _add_temporal_features(self, match_features: pd.DataFrame) -> pd.DataFrame:
        ...
        match_features["season_phase"] = np.where(
            match_features["month"] == 3,
            "early",
            np.where(match_features["month"] == 4, "mid", "late"),
        )

        # Weekend vs weekday
        match_features["is_weekend"] = (match_features["date"].dt.weekday >= 5).astype(
            int
        )
        ...
        if "season" in match_features.columns:
            ...
            match_features["days_since_season_start"] = match_features.apply(
                lambda row: (
                    (row["date"] - season_starts[row["season"]]).days
                    if row["season"] in season_starts.index
                    else 0
                ),
                axis=1,
            )
        ...
        match_features["match_number_in_season"] = (
            match_features.groupby("season").cumcount() + 1
        )
...
class DataPipeline:
    ...
    def preprocess_data(
        ...
        match_features = self.feature_engineer.create_match_features(
            matches_df, team_features
        )
        ...
        # Save processed data
        processed_path = self.config.get_data_path(
            "processed_features.csv", "processed"
        )
    ...
    def prepare_training_data(
        self, df: pd.DataFrame, target_col: str = "winner"
        ...
        # Select features
        feature_cols = [
            col for col in df.columns if col not in ["winner", "date", "id"]
        ]
