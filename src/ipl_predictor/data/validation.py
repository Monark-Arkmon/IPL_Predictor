...
class IPLDataValidator:
    ...
    VALID_IPL_TEAMS = {
        ...
        'Pune Warriors India', 'Deccan Chargers'
    }
    
    # Valid IPL cities/venues
    VALID_IPL_CITIES = {
        'Mumbai', 'Chennai', 'Bangalore', 'Delhi', 'Kolkata', 'Jaipur',
        'Hyderabad', 'Pune', 'Ahmedabad', 'Lucknow', 'Mohali', 'Indore',
        'Rajkot', 'Visakhapatnam', 'Guwahati', 'Ranchi', 'Dharamsala',
        'Kochi', 'Nagpur', 'Cuttack', 'Raipur', 'Kanpur'
    }
    ...
    def validate_matches_data(self, matches_df: pd.DataFrame) -> Tuple[bool, List[str], List[str]]:
    ...
    def validate_deliveries_data(self, deliveries_df: pd.DataFrame) -> Tuple[bool, List[str], List[str]]:
    ...
    def validate_combined_data(self, matches_df: pd.DataFrame, 
                             deliveries_df: pd.DataFrame) -> Tuple[bool, List[str], List[str]]:
    ...
    def _check_required_columns(self, df: pd.DataFrame, required_cols: List[str], dataset_name: str):
    ...
    def _validate_matches_data_types(self, df: pd.DataFrame):
        ...
        try:
            ...
            self.validation_errors.append("matches: 'date' column cannot be converted to datetime")
    ...
    def _validate_deliveries_data_types(self, df: pd.DataFrame):
        ...
        numeric_cols = ['match_id', 'inning', 'over', 'ball', 'batsman_runs', 
    ...
    def _validate_seasons(self, df: pd.DataFrame):
        ...
        if 'season' in df.columns:
            invalid_seasons = set(df['season'].dropna().unique()) - set(self.VALID_SEASONS)
            if invalid_seasons:
                self.validation_warnings.append(
    ...
    def _validate_team_consistency(self, matches_df: pd.DataFrame, deliveries_df: pd.DataFrame):
        """Validate team consistency between matches and deliveries."""
        # Get teams from both datasets
        match_teams = set(pd.concat([matches_df['team1'], matches_df['team2']]).unique())
        delivery_teams = set(pd.concat([
    ...
    def _check_data_quality(self, df: pd.DataFrame, dataset_name: str):
        ...
        logger.info(f"{dataset_name} quality check: {len(df)} records, "
                   f"{df.isnull().sum().sum()} total missing values")

def validate_ipl_data(matches_df: pd.DataFrame, deliveries_df: pd.DataFrame, 
    ...
    matches_valid, matches_errors, matches_warnings = validator.validate_matches_data(matches_df)
    deliveries_valid, deliveries_errors, deliveries_warnings = validator.validate_deliveries_data(deliveries_df)
    
    # Validate combined data
    combined_valid, combined_errors, combined_warnings = validator.validate_combined_data(
    ...
    return {
        ...
        'summary': {
            ...
            'unique_teams': len(set(pd.concat([matches_df['team1'], matches_df['team2']]).unique())),
            'seasons_covered': sorted(matches_df['season'].unique()) if 'season' in matches_df.columns else [],
            'date_range': (matches_df['date'].min(), matches_df['date'].max()) if 'date' in matches_df.columns else None
        }
    }