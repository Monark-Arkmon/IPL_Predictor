...
class IPLDataValidator:
    ...
    def load_datasets(self):
        ...
        # Main datasets
        self.main_ball_data = pd.read_csv('data/raw/ball_by_ball/ipl_ball_by_ball_2008_2024.csv')
        self.main_player_data = pd.read_csv('data/raw/players/ipl_players_2024.csv')
        
        # Alternative datasets
        self.alt_ball_data = pd.read_csv('data/external/ipl_ball_by_ball_alternative.csv', low_memory=False)
    ...
    def validate_ball_data_coverage(self):
        ...
        print("\n--- Season Coverage ---")
        try:
            main_seasons = sorted(self.main_ball_data['season'].unique()) if 'season' in self.main_ball_data.columns else []
            alt_seasons = self.alt_ball_data['Season'].unique() if 'Season' in self.alt_ball_data.columns else []
        ...
        # Match coverage
        main_matches = len(self.main_ball_data['match_id'].unique()) if 'match_id' in self.main_ball_data.columns else 0
        alt_matches = len(self.alt_ball_data['Match id'].unique()) if 'Match id' in self.alt_ball_data.columns else 0
    ...
    def validate_player_data(self):
        ...
        # Player overlap
        main_players = set(self.main_player_data['player_name'].str.lower()) if 'player_name' in self.main_player_data.columns else set()
        orig_players = set(self.orig_player_data['Player'].str.lower()) if 'Player' in self.orig_player_data.columns else set()
        ...
        if json_files:
            ...
            try:
                ...
                if 'info' in sample_data:
                    ...
                    useful_keys = ['city', 'venue', 'toss', 'player_of_match', 'outcome']
    ...
    def generate_integration_plan(self):
        ...
        if self.validation_results.get('ball_data'):
            ...
            if ball_results['alt_shape'][0] < ball_results['main_shape'][0]:
                print("    Recommendation: Use main dataset as primary, alternative for validation")
            else:
                print("    Recommendation: Compare data quality, potentially merge datasets")
        ...
        if self.validation_results.get('player_data'):
            ...
            if enhancement_cols:
                career_stats = [col for col in enhancement_cols if any(stat in col.lower() for stat in ['avg', 'sr', 'runs', 'wickets'])]
                print(f"   - Career statistics columns: {len(career_stats)}")
                print("    Recommendation: Merge career stats for enhanced player features")
