...
class IPLDataIntegrator:
    ...
    def fuzzy_match_players(self, threshold=80):
        ...
        for idx, current_name in enumerate(current_players):
            ...
            if best_match and best_match[1] >= threshold:
                historical_idx = historical_players[historical_players == best_match[0]].index[0]
                matches.append({
                    'current_idx': idx,
                    'historical_idx': historical_idx,
                    'current_name': self.main_player_data.iloc[idx]['Name'],
                    'historical_name': self.orig_player_data.iloc[historical_idx]['Player'],
    ...
    def enhance_player_data(self):
        ...
        enhancement_columns = [
            'Runs', 'Avg', 'SR', 'HS', '100s', '50s', '4s', '6s',  # Batting
            'B_Wkts', 'B_Avg', 'B_SR', 'B_Econ', 'B_5w', 'B_4w',  # Bowling
            'Mat', 'SOLD_PRICE', 'AGE', 'CAPTAINCY EXP'  # General
        ]
        ...
        for col in enhancement_columns:
            ...
            self.enhanced_players[col] = pd.to_numeric(self.enhanced_players[col], errors='coerce').fillna(0)
            
        print(f"Enhanced {len(matches)} players with career statistics")
        
        # Display sample enhancement
        enhanced_sample = self.enhanced_players[self.enhanced_players['Runs'] > 0].head(3)
        print("\\nSample enhanced players:")
        for _, player in enhanced_sample.iterrows():
            print(f"  {player['Name']}: {player['Runs']} runs, {player['Avg']:.1f} avg, {player['B_Wkts']} wickets")
    ...
    def extract_match_context_from_json(self, limit=100):
        ...
        if not self.match_context.empty:
            ...
            for _, match in self.match_context.head(3).iterrows():
                print(f"  Match {match['match_id']}: {match['venue']}, won by {match['outcome_winner']}")
