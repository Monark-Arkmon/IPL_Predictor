...
class TeamFeatureEngine:
    ...
    def extract_team_composition_features(self, team_players: List[str]) -> Dict[str, float]:
        ...
        batting_features = {
            ...
            'power_hitters': len(team_df[team_df['6s'].fillna(0) > 50]),  # Players with 50+ sixes
        ...
        bowling_features = {
            ...
            'avg_bowling_avg': team_df['B_Avg'].fillna(50).mean(),  # Default 50 for non-bowlers
            'avg_economy': team_df['B_Econ'].fillna(8).mean(),  # Default 8 for non-bowlers
            'total_5w_hauls': team_df['B_5w'].fillna(0).sum(),
            'quality_bowlers': len(team_df[team_df['B_Avg'].fillna(50) < 25]),  # Good bowling average
        ...
        balance_features = {
            ...
            'allrounders': len(team_df[
                (team_df['Runs'].fillna(0) > 500) & (team_df['B_Wkts'].fillna(0) > 10)
            ]),
            'premium_players': len(team_df[team_df['SOLD_PRICE'].str.contains('cr', na=False)]),
        ...
        financial_features = {
            ...
            'value_distribution': team_df['SOLD_PRICE'].apply(self._convert_price_to_float).std()
        }
        
        # Combine all features
        all_features = {**batting_features, **bowling_features, **balance_features, **financial_features}
        return all_features
    ...
    def _get_default_composition_features(self) -> Dict[str, float]:
        ...
        return {
            'avg_career_runs': 1000.0, 'avg_batting_avg': 25.0, 'avg_strike_rate': 120.0,
            'total_centuries': 2.0, 'total_fifties': 10.0, 'power_hitters': 3.0, 'consistent_batters': 4.0,
            'avg_wickets': 50.0, 'avg_bowling_avg': 30.0, 'avg_economy': 7.5,
            'total_5w_hauls': 1.0, 'quality_bowlers': 4.0, 'economical_bowlers': 3.0,
            'avg_experience': 75.0, 'veteran_players': 3.0, 'young_players': 4.0,
            'allrounders': 2.0, 'premium_players': 6.0, 'captaincy_options': 2.0,
            'team_value_estimate': 85.0, 'value_distribution': 5.0
        }
    
    def extract_team_form_features(self, matches_data: pd.DataFrame, team: str, 
                                  current_date: str, lookback: int = 5) -> Dict[str, float]:
        ...
        ].sort_values('match_date', ascending=False).head(lookback)
    ...
    def extract_playing_xi_features(self, match_data: pd.Series, team: str) -> Dict[str, float]:
    ...
    def extract_head_to_head_team_features(self, matches_data: pd.DataFrame,
    ...
    def extract_venue_advantage_features(self, matches_data: pd.DataFrame,
                                       team: str, venue: str) -> Dict[str, float]:
    ...
    def extract_toss_strategy_features(self, matches_data: pd.DataFrame,
                                     team: str, venue: str) -> Dict[str, float]:
        """Analyze team's toss decisions and success."""
        team_toss_matches = matches_data[
            (matches_data['toss_winner'] == team) &
            (matches_data['venue'] == venue)
        ]
        
        if len(team_toss_matches) == 0:
            return {'toss_wins': 0, 'bat_first_preference': 0.5, 'toss_win_success': 0.5}
    ...
    def _get_default_team_features(self) -> Dict[str, float]:
        ...
        return {
            'recent_win_rate': 0.5, 'recent_matches': 0,
            'form_momentum': 0.5, 'consistency': 0.5
        }
    
    def _get_default_xi_features(self) -> Dict[str, float]:
        """Default playing XI features."""
        return {
            'team_size': 11, 'team_balance_score': 0.7,
            'experience_factor': 0.6, 'star_player_count': 3
        }