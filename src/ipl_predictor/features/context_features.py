...
class ContextFeatureEngine:
    ...
    def _load_match_context(self) -> pd.DataFrame:
        ...
        if context_path.exists():
            ...
            logger.warning(
                "Enhanced match context not found, using basic features only"
            )
            return pd.DataFrame()

    def extract_venue_features(
        self, matches_data: pd.DataFrame, venue: str
    ) -> Dict[str, float]:
        ...
        if len(venue_matches) == 0:
            ...
            bat_first_decisions = len(
                venue_matches[venue_matches["toss_decision"] == "bat"]
            )

            features = {
                "total_matches_played": total_matches,
                "bat_first_advantage": (
                    (toss_win_bat_first / bat_first_decisions)
                    if bat_first_decisions > 0
                    else 0.5
                ),
                "field_first_advantage": (
                    (toss_win_field_first / (total_matches - bat_first_decisions))
                    if (total_matches - bat_first_decisions) > 0
                    else 0.5
                ),
                "toss_importance": (toss_win_bat_first + toss_win_field_first)
                / total_matches,
    ...
    def _extract_enhanced_venue_features(self, venue: str) -> Dict[str, float]:
        ...
        if total_decisions > 0:
            enhanced_features["enhanced_bat_first_rate"] = (
                toss_bat_decisions / total_decisions
            )
        ...
        if total_toss_data > 0:
            enhanced_features["enhanced_toss_advantage"] = (
                toss_wins_match / total_toss_data
            )
    ...
    def extract_tournament_context_features(
        self, matches_data: pd.DataFrame, match_date: str, team1: str, team2: str
    ) -> Dict[str, float]:
        ...
        played_matches = current_season_matches[
            current_season_matches["match_date"] < current_date
        ]
        ...
        features = {
            ...
            "elimination_context": self._get_elimination_context(
                match_date, matches_data
            ),
        }

        return features

    def extract_rest_and_fatigue_features(
        self, matches_data: pd.DataFrame, match_date: str, team1: str, team2: str
    ) -> Dict[str, float]:
    ...
    def _get_elimination_context(
        self, match_date: str, matches_data: pd.DataFrame
    ) -> float:
    ...
    def _get_last_match_date(
        self, matches_data: pd.DataFrame, team: str, current_date: datetime
    ):
    ...
    def _get_default_venue_features(self) -> Dict[str, float]:
        ...
        return {
            ...
            "bat_first_advantage": 0.5,
            "field_first_advantage": 0.5,
            "toss_importance": 0.5,
