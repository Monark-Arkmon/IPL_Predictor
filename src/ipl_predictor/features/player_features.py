...
class PlayerFeatureEngine:
    ...
    def get_enhanced_player_features(self, player_name: str) -> Dict[str, float]:
        ...
        features = {
            ...
            'experience_level': self._encode_experience(player.get('experience_level', 'Developing')),
            
            # Performance indicators
            'is_premium_player': 1.0 if self._convert_auction_price(player.get('SOLD_PRICE', '0')) > 5.0 else 0.0,
            'is_experienced': 1.0 if float(player.get('Mat', 0)) > 50 else 0.0,
            'is_allrounder': 1.0 if (float(player.get('Runs', 0)) > 500 and float(player.get('B_Wkts', 0)) > 10) else 0.0
        }
    ...
    def extract_batting_features(self, ball_data: pd.DataFrame, player_name: str, 
                                lookback_matches: int = 10) -> Dict[str, float]:
        ...
        current_features = {
            ...
            'current_strike_rate': (player_balls['BatsmanRun'].sum() / len(player_balls)) * 100,
            'recent_avg': recent_balls['BatsmanRun'].mean(),
            'recent_strike_rate': (recent_balls['BatsmanRun'].sum() / len(recent_balls)) * 100,
            'boundary_rate': len(player_balls[player_balls['BatsmanRun'].isin([4, 6])]) / len(player_balls),
            'dot_ball_rate': len(player_balls[player_balls['BatsmanRun'] == 0]) / len(player_balls),
        ...
        if features.get('career_avg', 0) > 0:
            features['form_vs_career'] = current_features['current_batting_avg'] / features['career_avg']
        else:
            features['form_vs_career'] = 1.0
            
        return features
    
    def extract_bowling_features(self, ball_data: pd.DataFrame, player_name: str,
                                lookback_matches: int = 10) -> Dict[str, float]:
        ...
        recent_balls = player_balls.tail(lookback_matches * 24)  # ~24 balls per match for bowler
        ...
        current_features = {
            'current_economy_rate': total_runs / total_overs if total_overs > 0 else 8.0,
            ...
            'recent_wicket_rate': recent_wickets / recent_overs if recent_overs > 0 else 0.0,
            'dot_ball_percentage': len(player_balls[player_balls['TotalRun'] == 0]) / len(player_balls),
            'boundary_conceded_rate': len(player_balls[player_balls['BatsmanRun'].isin([4, 6])]) / len(player_balls),
        ...
        if features.get('career_economy', 0) > 0:
            features['economy_vs_career'] = current_features['current_economy_rate'] / features['career_economy']
        else:
            features['economy_vs_career'] = 1.0
            
        return features
    
    def extract_head_to_head_features(self, ball_data: pd.DataFrame, 
                                     batter: str, bowler: str) -> Dict[str, float]:
        ...
        if len(h2h_balls) == 0:
            return {'h2h_balls': 0, 'h2h_avg': 0.0, 'h2h_strike_rate': 0.0, 'h2h_dismissals': 0}
    ...
    def extract_venue_features(self, ball_data: pd.DataFrame, matches_data: pd.DataFrame,
                              player_name: str, venue: str) -> Dict[str, float]:
        """Extract player performance at specific venue."""
        venue_matches = matches_data[matches_data['venue'] == venue]['match_number'].values
        venue_balls = ball_data[
            (ball_data['ID'].isin(venue_matches)) & 
            ((ball_data['Batter'] == player_name) | (ball_data['Bowler'] == player_name))
        ]
        ...
        if player_name in venue_balls['Batter'].values:
            ...
            runs_conceded = bowling_balls['BatsmanRun'].sum() + bowling_balls['ExtrasRun'].sum()
            venue_perf = runs_conceded / (len(bowling_balls) / 6) if len(bowling_balls) > 0 else 8.0
        
        return {
            'venue_experience': len(venue_balls),
    ...
    def _get_default_batting_features(self) -> Dict[str, float]:
        ...
        return {
            'batting_avg': 0.0, 'strike_rate': 100.0, 'recent_avg': 0.0,
            'recent_strike_rate': 100.0, 'boundary_rate': 0.1, 'dot_ball_rate': 0.5,
            'consistency': 2.0, 'total_balls_faced': 0
        }
    
    def _get_default_bowling_features(self) -> Dict[str, float]:
        """Default features for players with no bowling data."""
        return {
            'economy_rate': 8.0, 'wicket_rate': 0.0, 'recent_economy': 8.0,
            'recent_wicket_rate': 0.0, 'dot_ball_percentage': 0.3,
            'boundary_conceded_rate': 0.2, 'total_balls_bowled': 0
        }