...
class FeaturePipeline:
    ...
    def extract_match_features(
        ...
        try:
            ...
            team1_composition = self.team_engine.extract_team_composition_features(
                [team1]
            )  # Simplified
            team2_composition = self.team_engine.extract_team_composition_features(
                [team2]
            )  # Simplified
        ...
        try:
            ...
            team1_player_features = self._extract_enhanced_player_features(
                ball_data, team1_players[:3]
            )  # Top 3
            team2_player_features = self._extract_enhanced_player_features(
                ball_data, team2_players[:3]
            )  # Top 3
        ...
        if toss_winner and toss_decision:
            features.update(
                self._extract_toss_features(toss_winner, toss_decision, team1, team2)
            )
    ...
    def extract_training_dataset(
        ...
        logger.info(
            f"Created training dataset with {len(feature_df)} samples and {len(feature_df.columns)} features"
        )
    ...
    def _extract_toss_features(
        self, toss_winner: str, toss_decision: str, team1: str, team2: str
    ) -> Dict[str, float]:
    ...
    def _get_key_players_for_team(
        self, ball_data: pd.DataFrame, team: str
    ) -> List[str]:
        """Get key players for a team based on performance data."""
        try:
            # Get players who have played for this team (simplified approach)
            team_players = ball_data[ball_data["BattingTeam"] == team][
                "Batter"
            ].value_counts()
            team_bowlers = ball_data[ball_data["BowlingTeam"] != team][
                "Bowler"
            ].value_counts()
    ...
    def _extract_enhanced_player_features(
        self, ball_data: pd.DataFrame, players: List[str]
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for player in players:
                player_features = self.player_engine.get_enhanced_player_features(
                    player
                )
                if player_features:
                    player_count += 1
