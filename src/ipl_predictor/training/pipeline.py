...
class TrainingPipeline:
    ...
    def prepare_training_data(self, min_date: str = "2015-01-01", 
    ...
    def train_xgboost_model(self, X: pd.DataFrame, y: pd.Series,
        ...
        logger.info(f"XGBoost CV Accuracy: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
        
        return results
    
    def train_neural_network_model(self, X: pd.DataFrame, y: pd.Series,
                                 model_params: Optional[Dict] = None) -> Dict:
        ...
        nn_model = NeuralNetworkPredictor(hidden_layers=hidden_layers, dropout_rate=dropout_rate)
    ...
    def generate_training_report(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        report = {
            'data_summary': {
                ...
                'class_balance': {str(k): float(v) for k, v in y.value_counts(normalize=True).to_dict().items()},
                'feature_types': {str(k): int(v) for k, v in X.dtypes.value_counts().to_dict().items()}
            },
            'top_features': {},
            'model_comparison': {}
        }
    ...
    def load_and_evaluate_model(self, model_path: str, X_test: pd.DataFrame, 
